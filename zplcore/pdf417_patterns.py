"""The PDF417 low-level pattern table, from ISO/IEC 15438.

Every codeword is seventeen modules - four bars and four spaces, each one to
six modules wide, always starting with a bar. Which pattern a codeword takes
depends on its row: rows cycle through three clusters, and a reader tells one
row from the next by which cluster it is reading. The cluster a pattern
belongs to is (b1 - b2 + b3 - b4) mod 9 over its bar widths, which is checked
in the tests rather than assumed here.

These are the standard's own numbers and are identical in every
implementation of the symbology; there is no other set of them. Each is the
seventeen module bits, most significant first, so 0x1d5c0 is 11101010111000000
- a three-module bar, then single modules, then a three-module bar and six of
space.
"""

# Indexed by the row's cluster (0, 1 or 2, for the standard's 0, 3 and 6),
# then by the codeword's value, 0 to 928.
PATTERNS = (
    (
...
# The patterns either end of every row. The stop pattern is eighteen modules,
# not seventeen: the extra one is the bar that closes the symbol.
START = 0x1fea8
STOP = 0x3fa29
CODEWORD = 17
STOP_WIDTH = 18
