...
class DesignCanvas(QWidget):
    ...
    def _draw_text_fallback(self, painter, element, font_path, reverse=False):
        ...
        if font_path:
            ...
            # on-screen footprint (sync_text_width), so element.width is the
            # run along the text only when the field is upright. printed_width()
            # measures along the text itself, so font_width still stretches the
            # preview once the field is rotated.
            target_width = element.printed_width(font_path, shown)
            h_scale = target_width / measured
